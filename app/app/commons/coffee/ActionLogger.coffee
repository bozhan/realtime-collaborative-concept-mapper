...
class window.ut.commons.actionlogging.ActionLogger
  ...
  setLoggingTargetByName: (newLoggingTargetName) ->
    ...
    else if newLoggingTargetName is "server" then @loggingTarget = @serverLogging
    else
  ...
  serverLogging: (activityStreamObject) ->
    http = location.protocol;
    slashes = http.concat("//");
    host = slashes.concat(window.location.hostname);
    subDir = "/log"
    destinationUrl = host + ":" + location.port + subDir
    console.log "ActionLogger: logging to #{destinationUrl}: #{activityStreamObject.verb} #{activityStreamObject.object.objectType}, id: #{activityStreamObject.object.id}"
    $.ajax({
      type: "POST",
      url: destinationUrl,
      data: JSON.stringify(activityStreamObject),
