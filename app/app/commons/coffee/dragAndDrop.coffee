...
ut.commons.golabUtils.directive("draggable", () ->
  {
  restrict: "A"
...
ut.commons.golabUtils.directive("droppable", ($compile) ->
  {
  restrict: "A"
  link: (scope, element, attrs)->
    ...
    acceptDrop = (draggable)->
      ...
      if (scope.acceptObjectDrop)
        dropElement = angular.element(draggable)
        scope.acceptObjectDrop(collectDraggableInformation(dropElement, element, attrs), dropElement)
      else
        true

    objectDropped = (event, ui)->
      dropElement = angular.element(ui.helper)
      scope.objectDroppedInside?(collectDraggableInformation(dropElement, element, attrs), dropElement)
      stopShowDropAllowedState(dropElement)
      scope.$apply()
